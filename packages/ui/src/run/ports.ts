...
/** What a saved edit or a rollback answers: whether it took, and which version is live. */
...
/** The rollback list's data (§8.1): the history plus the version currently in effect. */
...
export interface RunMessagingPorts {
  ...
  /**
   * The config tab's save (stage 1-16).
   *
   * It goes through `build:save_tool` with an explicit `repair` request — **never** a
   * bare save. A bare save of an existing tool *merges* into the record it finds, which
   * would rewrite the definition in place; only the repair branch appends a version and
   * keeps the old one. `trigger: 'user'` is what stops that new version from marking the
   * one it replaced as broken: editing a working tool is not evidence that it failed.
   */
  saveEdit(input: { toolId: string; tool: ToolDefinition }): Promise<WriteReply>
  /** `versions[]` lives on the record and `run:query_tools` answers definitions only. */
  loadVersions(toolId: string): Promise<VersionHistory | null>
  rollback(input: { toolId: string; version: number }): Promise<WriteReply>
}
...
export function createRunPorts(adapter: BrowserAdapter): RunMessagingPorts {
  return {
    ...
    async saveEdit({ toolId, tool }): Promise<WriteReply> {
      const reply = await adapter.messaging.send({
        kind: 'build:save_tool',
        tool,
        repair: { toolId, trigger: 'user', note: t('run.repair.note_edit') },
      })
      if (reply === null || reply.kind !== 'build:save_tool_result') return { ok: false }
      return reply
    },

    async loadVersions(toolId: string): Promise<VersionHistory | null> {
      const reply = await adapter.messaging.send({ kind: 'tool:get', toolId })
      if (reply === null || reply.kind !== 'tool:get_result') return null
      return { version: reply.version, versions: reply.versions }
    },

    async rollback({ toolId, version }): Promise<WriteReply> {
      const reply = await adapter.messaging.send({ kind: 'tool:rollback', toolId, version })
      if (reply === null || reply.kind !== 'tool:rollback_result') return { ok: false }
      return reply
    },
  }
}
