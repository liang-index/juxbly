...
- **Benchmark runner** (`apps/playground/src/bench/`, `pnpm test:bench`) — runs the real pipeline over the corpus (snapshot → page analysis → propose → execute) and writes an immutable `results/<run-id>.json` plus a report carrying the corpus revision, model, date, failure excerpts and a delta against the previous run. Metrics are Build Success Rate, correct / partial / wrong, correction rate, health false-positive rate, latency and tokens, every one of them split by bucket. The runner never judges a result itself: labels come from `results/labels.json`, and unlabelled cases count as `pending`. Without an API key it prints SKIPPED and exits 0. CI runs a 5-case slice on pull requests that touch the shared core; the full run is `workflow_dispatch`.
